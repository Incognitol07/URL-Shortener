...
@url_router.get("/admin/{url_key}", response_model=URLInfo, name="Admin Info")
def get_url_info(
    url_key: str, 
    request: Request,
    db: Session = Depends(get_db),
...
@url_router.delete("/admin/{url_key}")
def delete_url(
    url_key: str, 
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user)
    ):
    db_url = deactivate_db_url_by_key(db, url_key, user)
    if not db_url:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"URL with key {url_key} not found."
        )
